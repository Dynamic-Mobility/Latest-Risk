...
const currencies = [{id: 1, name:"KES"},{id: 2, name:"USD"}, {id: 3, name:"EURO"}, ];
...
router.get('/frequency/risk', async (req, res) => {
    ...
    try{
        await needle('get',`${API_URL}/keyfrequency/risk`, options).then(response => {
...
router.get('/risk-appetite-types', async (req, res) => {
    ...
    try{
        await needle('get',`${API_URL}/riskappetitetype`, options).then(response => {
            res.status(response.statusCode).json(response.body);
        }).catch(error => res.json(error))
