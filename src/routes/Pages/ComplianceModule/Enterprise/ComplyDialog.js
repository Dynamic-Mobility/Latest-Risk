...
import { getEnterpriseComplianceSub } from "../../../../Redux/features/Compliance/enterprise";
...
export default function ComplyDialog(props) {
  ...
  const handleComply = async () => {
    ...
    try {
      const res = await enterpriseApi.complyEnterpriseCompliance(dispatch,formData);
      if (res.success) {
        toast.success("Complied Successfully!");
        dispatch(getEnterpriseComplianceSub({id:data.id}))
        handleClose();
      } else {
    ...
    // dispatch(enterpriseApi.complyEnterpriseCompliance(formData));
