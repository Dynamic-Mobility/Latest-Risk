...
const getActions = (permissions) => {
  ...
  if (
    validatePermission(PERMISSIONS.STATUTORY_COMPLIANCE.UPDATE, permissions)
  ) {
    actions.push({ action: "edit", label: "Edit", icon: <Edit /> });
  }
  if (
    validatePermission(PERMISSIONS.STATUTORY_COMPLIANCE.DELETE, permissions)
  ) {
...
const initialBreadcrumbs = [
  ...
  { label: HEADER.STATUTORY_TABLE, isActive: true },
];
...
const StatutoryComplianceTable = () => {
  ...
  const { statutoryMainComplianceData, statutoryComplianceSub } = useSelector(
    ({ statutory }) => statutory
  );
  ...
  const steps = statutoryMainComplianceData;

  const [activeStep, setActiveStep] = React.useState(0);

  //-------------DOTTED ACTIONS---------------//
  const onEditCompliance = (data) => {
    navigate("update-statutory", { state: data });
  };

  const onMenuClick = async (menu, data) => {
    if (menu.action === "edit") {
      onEditCompliance(data);
    } else if (menu.action === "delete") {
      try {
        dispatch(setLoading(true));
        console.log("Calling deleteStatutoryCompliance API");
        await complianceApi.deleteStatutoryCompliance(dispatch,{
          id: data.id,
        });
  ...
  const displaySubData = async (id, index) => {
    await dispatch(getStatutoryComplianceSub({ id }));
    setActiveStep(index);
  };
  ...
  const approveStatutory = async (id) => {
    try {
      const res = await complianceApi.approveStatutoryCompliance(dispatch, {
        id,
      });
      if (res.success) {
        toast.success(res.message);
        dispatch(getStatutoryComplianceSub({ id }));
      }
    } catch (error) {
      toast.error(error);
    }
  ...
  const approveCompliance = ({ displayValue, data }) => {
    if (data.approvalStatus === "Approved") {
      ...
      return (
        <Chip
          onClick={() => approveStatutory(data.id)}
          label={displayValue}
          variant="outlined"
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(statutoryComplianceSub);
